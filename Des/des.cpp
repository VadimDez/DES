...
void Des::on_start_clicked()
{
    char *str=new char[1000];
    char *str1=new char[1000];
    strcpy(str,ui->text->text().toStdString().c_str());
    if(ui->cod->isChecked())
    {
        Des encrypt;
        str1=encrypt.Encrypt(str);
        ui->result->setText(str1);
    }
    else if(ui->decod->isChecked())
    {
        Des decrypt;
        ui->result->setText(decrypt.Decrypt(str));
    }
